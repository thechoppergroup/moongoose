...
module (`main: dist/moongoose.js`). It is the **bottom of the dependency
chain** — `moonscape` and every app (`web`, `moonshine`, `wsmobile`,
`ws_desktop`) consume it. Published to the private npm registry.

## Commands
- `npm run build` — clean → `scripts/svg-to-module.js` (bundle SVGs) →
  webpack. Output to `dist/`.
- `npm run watch` — rebuild the module + webpack `--watch`.
- `npm run build-docs` / `npm run deploy-docs` — build the docs site / deploy
  it to Netlify.
